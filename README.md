...
D日特征行情优先读取 Eastmoney；传输不可用时可切换至腾讯只读日线。实际使用的供应商、复权口径和样本数会随冻结信号持久化，后续重跑不会丢失这项审计证据。T日结果只接受目标日期精确匹配的未复权收盘日线，并在北京时间15:10之后结算；退出执行只接受未复权分钟数据。工作流当前不会自动生成9:25精确竞价成交真值；只有 `execution_truth.v1.json` 中通过券商回报、交易所逐笔回放或合格队列证据写入的记录才能成为入场真值。日期、价、量、额或时间语义不能验证时均失败关闭，不伪造结果。
