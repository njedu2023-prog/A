...
两层契约已接入新D日信号的冻结流程。系统先完成严格交集、冠军排名和 `shadow_order_spec_v1`，再生成候选的 `single_stock_research_audit_v1`；因此单票研究成功、失败或缺失都不能反向改变交集、名次或订单。构建失败必须落一份有明确原因和SHA-256指纹的 `UNAVAILABLE` 信封，不能退化为空对象；旧冻结信号不事后回填或重算，看板对其保持 `null`。

每支严格交集候选的可用审计对象内含一份 `single_stock_snapshot_v3`、D日候选分析白名单摘要、冻结订单摘要和D日分钟状态代理。`single_stock_snapshot_v3` 保存候选标识、三源名次、D日冻结时点、逐字段来源、`known_at`、抓取时间、内容哈希、缺失原因与硬门控结果。整个研究信封的指纹覆盖所有嵌套字段；分钟输入另以规范化日期、时间、OHLC、量额、最小价位、来源、复权和时间语义计算内容指纹。当前未保存全部240根原始分钟行，因此指纹可检测内容变化但不能替代独立原始证据仓。它与 `features`、`metrics` 及 `shadow_order_spec_v1` 分离保存；训练数据集明确忽略整个单票审计对象，未经新特征契约、前推验证与模型晋级不得进入冠军模型。

研究硬门控的唯一合法状态为：

- `PASS`：所需证据完整，且没有命中已定义的确定性阻断条件。
- `BLOCK`：证据足以确认停牌、退市整理、交易规则冲突等确定性风险。
...
当前尚未连接可按D时点查询的证券主数据，停牌、退市整理和个股交易规则三项必须保存为缺失事实。因此首阶段正常候选通常为 `UNKNOWN`；不得把这一版本描述成已经具备完整停牌/退市硬门控能力。
...
生产采集使用独立的短预算：单请求最多3秒、只尝试1次、最多5票并行。抓取或构建异常只能形成候选级 `UNAVAILABLE` 警告，不得改变交集、排名或冻结订单；配置若试图让该模块影响交集、排名、订单或声称真实竞价门控已连接，启动时必须失败关闭。
