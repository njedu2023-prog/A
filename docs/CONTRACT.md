...
`T日涨跌幅 / 是否涨停 / 是否晋级` 与9:25是否成交相互独立：即使候选最终未成交，也必须验证股票在T日的真实表现。北京时间T日15:10之前不得请求或写入收盘真值，页面统一显示“待验证”；15:10之后及后续日期可重试，证据不足时显示“待补证”，不能写成“否”。

验证只接受日期严格等于T且 `price_adjustment=NONE` 的无复权日线。`T日涨跌幅` 优先使用供应商正式涨跌幅并与 `T收盘价 / 冻结D收盘价 - 1` 交叉校验；供应商未提供时使用后者。`是否涨停` 以T日收盘价在半个最小价位内等于冻结涨停价判定；盘中触及后开板不会被记成收盘涨停。`是否晋级` 只有在合法晋级目标存在且T日收盘涨停时为“是”。最终真值一经写入即保持幂等，不因后续供应商数据修订而静默改写。
