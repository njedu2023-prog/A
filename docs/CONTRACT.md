...
自动调度先以版本化的上交所交易日历验证上海本地日期；休市日不得请求行情、读取三源或发布伪造批次。交易日分为两个职责隔离的批次：北京时间19:00只处理已冻结名单的T日收盘验证和到期T+1退出，禁止读取三源、求新交集或产生新Signal；21:30才进入三源就绪轮询并冻结D日输出名单。两个批次都必须在发布前通过结果自检；21:30批次仍会幂等重试待处理验证，但不得篡改19:00批次的实际执行时间。
